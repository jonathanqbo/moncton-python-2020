...
draw_text(-270, 200, 'Guess Number  (猜数字)', size=36)
draw_text(-300, 110, 'Hello everyone, I am thinking a number between 1 and '
          + str(MAX_TARGET_NUMBER) + '\n (大家好, 我在想一个数字在 1 和' + str(MAX_TARGET_NUMBER) + '之间' +
          '。 \nYou have 5 chances to guess it.(你有 5 次机会去猜。) \nCan you get it?(你能猜到吗？）')
indicator_turtle = create_turtle(0, 0, color='red', speed='fast')

used_chances = 0
many_show_guessed_number = []
target_number = random.randint(0, MAX_TARGET_NUMBER)
while used_chances < 4:
    guessed_number = player_input_number()
    many_show_guessed_number.append(guessed_number)
...
draw_text(-120, -150, 'The target number is:  ' + str(target_number))
draw_text(-170, -220, 'Your guessed number is: ' + str(many_show_guessed_number))
