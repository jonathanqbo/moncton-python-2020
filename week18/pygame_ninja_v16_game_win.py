...
class GameWinSign(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.colliderect(self.__game.ninja.rect):
            self.__game.game_win()
...
class Level1(Level):
    ...
    def build(self):
        ...
        self.game.all_sprites.add(first_ground_tile, layer=10)
        self.game.platforms.add(first_ground_tile)

        for i in range(5):
            Water(self.game, SCREEN_HEIGHT, 3, 1, self.game.decorators)
            grass_platform = GrassPlatform(self.game, SCREEN_HEIGHT, 3, 1, self.game.platforms)
            Decorator(GrassPlatformDecoration).decor(grass_platform)
            Bat(grass_platform.rect.centerx, grass_platform.rect.top - 200, self.game.enemies)

        platform = GrassPlatform(self.game, SCREEN_HEIGHT, 3, 1, self.game.platforms)
        NextLevelSign(self.game, platform.rect.right - 100, platform.rect.top, self.game.decorators)
...
class Level2(Level):
    ...
    def build(self):
        ...
        water = Water(self.game, SCREEN_HEIGHT, 20, 1, self.game.decorators)
        for i in range(7):
            platform = GrassFloatPlatform(self.game, water.rect.left + i * 450, 600 - i * 50, 2, self.game.platforms)
            Decorator(GrassPlatformDecoration).decor(platform)
            Snake(platform.rect.centerx, platform.rect.top, self.game.enemies)

        platform = GrassPlatform(self.game, SCREEN_HEIGHT, 3, 1, self.game.platforms)
        GameWinSign(self.game, platform.rect.right - 100, platform.rect.top, self.game.decorators)
...
class Game:
    ...
    def game_over(self):
        ...
        self.all_sprites.remove(self.ninja)
    ...
    def game_win(self):
        gamewin = GameWin(self)
        pygame.time.wait(1000)

        running = True
        clock = pygame.time.Clock()
        self.all_sprites.empty()
        self.all_sprites.add(gamewin)
        while running:
            clock.tick(FPS)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
